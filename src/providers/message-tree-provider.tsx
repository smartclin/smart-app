...
import { buildThreadFromLeaf, findLeafDfsToRightFromMessageId } from '@/lib/thread-utils'
...
export function MessageTreeProvider({ children }: MessageTreeProviderProps) {
	...
	useEffect(() => {
		...
		return unsubscribe
	}, [id, type, trpc.chat.getChatMessages, trpc.chat.getPublicChatMessages, queryClient])
	...
	const navigateToSibling = useCallback(
		(messageId: string, direction: 'prev' | 'next') => {
			...
			const leaf = findLeafDfsToRightFromMessageId(childrenMap, targetSibling.id)
			const newThread = buildThreadFromLeaf(allMessages, leaf ? leaf.id : targetSibling.id)
	...
	return <MessageTreeContext.Provider value={value}>{children}</MessageTreeContext.Provider>
}
