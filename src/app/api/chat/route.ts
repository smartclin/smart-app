...
import { REASONING_SYSTEM_PROMPT, SYSTEM_PROMPT } from '@/config'
import { db } from '@/db'
...
import { convertToAISDKMessages, extractText, injectCurrentDate } from '@/lib/utils'
...
export async function GET(req: Request) {
	...
	const streamWithMessage = streamText({
		execute(buffer) {
			buffer.writeData({
				type: 'append-message',
...
export async function POST(req: Request) {
	...
	const finalSystemPrompt =
		tool === 'reasoning' ? REASONING_SYSTEM_PROMPT : injectCurrentDate(SYSTEM_PROMPT, timezone)
		...
		execute: dataStream => {
			...
			const result = streamText({
				...
				async onFinish({ response }) {
					...
					try {
						const assistantMessages = StreamingTextResponse({
							messages,
								...
								try {
									...
									await db.message.create({
										data: {
											...
											parts: JSON.parse(JSON.stringify(lastAssistantMessage.parts ?? [])),
											imageUrl,
											imageKey,
