...
import { imageModelsFeatures, type ModelFeatures, modelFeatures } from './model-features'
...
export const chatModels = allModels
	.filter(
		model => !model.disabled && (model.features?.output?.text === true || !model.features), // Let's show models even if we haven't created features yet
	)
	.sort((a, b) => {
		...
		const aIndex = aProviderIndex === -1 ? PROVIDER_ORDER.length : aProviderIndex
		const bIndex = bProviderIndex === -1 ? PROVIDER_ORDER.length : bProviderIndex
...
export function getImageModelDefinition(modelId: ImageModelId): ModelDefinition {
	const modelsByIdDict = getModelsByIdDict()
	const model = modelsByIdDict.get(modelId)
	if (!model) {
		throw new Error(`Model ${modelId} not found`)
...
export const DEFAULT_CHAT_IMAGE_COMPATIBLE_MODEL: ModelId = 'openai/gpt-4o-mini'
